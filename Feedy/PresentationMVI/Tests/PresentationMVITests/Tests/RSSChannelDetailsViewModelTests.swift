...
@testable import PresentationMVI
...
final class RSSChannelDetailsViewModelTests: XCTestCase {
    ...
    @MainActor private func resetAll() async {
        ...
        observationTracker = .init(object: sut, keyPath: \.state.status)
    ...
    @MainActor func test_observeEnvironment_givenMatchingHistoryItem_thenUpdatesStates() async throws {
        ...
        await yieldTasks()
        // Then
        XCTAssertEqual(sut.state.isFavourite, false)  // Original item is true, this means it changed it.
    }
    ...
    @MainActor func test_environment_givenNoMatchingHistoryItem_thenDoesNothing() async throws {
        ...
        await yieldTasks()
        // Then: does nothing
        XCTAssertEqual(sut.state.isFavourite, true) // Original item is true, this means it didn't changed it.
    }
